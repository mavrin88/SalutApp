    ...
    public function boot()
    {
        if (core()->getConfigData('marketplace.settings.general.status')) {
            ...
            if ( (core()->getCurrentChannel() && core()->getCurrentChannel()->theme == "velocity")) {
                ...
                Event::listen('bagisto.shop.layout.head', function($viewRenderEventManager) {
                    $viewRenderEventManager->addTemplate('marketplace::shop.default.layouts.style');
                });
            ...
            Event::listen('catalog.product.update.after', 'Webkul\Marketplace\Listeners\Product@afterAdminProductUpdate');

            Event::listen('marketplace.seller.profile.update.after','Webkul\Marketplace\Listeners\Customer@afterSellerUpdate');
            ...
            Event::listen('core.configuration.save.after', 'Webkul\Marketplace\Listeners\EnableDisable@manageConfig');

            Event::listen( [ 'marketplace.sellers.account.catalog.products.assign.after', 
            'marketplace.sellers.account.catalog.product.edit-assign.after' ],
                function ($viewRenderEventManager) {
                    $viewRenderEventManager->addTemplate('marketplace::admin.sellers.products.accordians.group_product');
                }
            );
        
            Event::listen( [ 'marketplace.sellers.account.catalog.products.assign.product_image.before' ],
                function ($viewRenderEventManager) {
                    $viewRenderEventManager->addTemplate('marketplace::shop.sellers.account.catalog.products.accordians.admin_group_product_section');
                }
            ); 

            Event::listen( [ 'marketplace.sellers.account.catalog.product.edit_form_accordian.inventories.before' ],
                function ($viewRenderEventManager) {
                    $viewRenderEventManager->addTemplate('marketplace::shop.sellers.account.catalog.products.accordians.group_section');
                }
            );

            Event::listen( [ 'marketplace.sellers.account.catalog.product.assign.image.before', 'marketplace.sellers.account.catalog.product.edit-assign.after' ],
                function ($viewRenderEventManager) {
                    $viewRenderEventManager->addTemplate('marketplace::admin.sellers.products.accordians.bundle_product');
                }
            );
            
            Event::listen( [ 'marketplace.sellers.account.catalog.product.edit_form_accordian.bundle_product.before' ],
                function ($viewRenderEventManager) {
                    $viewRenderEventManager->addTemplate('marketplace::shop.sellers.account.catalog.products.accordians.bundle_product_section');
                }
            );
            
            // After assiging assign products or after updatinf assign products
            Event::listen( ['marketplace.catalog.assign-product.update.after', 'marketplace.catalog.assign-product.create.after'], 'Webkul\Marketplace\Listeners\Product@saveProductDetails');
        }
    }
}
