...
@if($product->type == 'simple' || $product->type == 'configurable')
    ...
    <div class="{{ $toolbarHelper->isModeActive('grid') ? 'cart-wish-wrap' : 'default-wrap' }}">
        <form action="{{ route('shop.cart.add', $product->product_id) }}" method="POST">
            ...
            <button class="btn btn-lg btn-primary addtocart" {{ $InventoryHelper->isSaleable($product) ? '' : 'disabled' }}>{{ ($product->type == 'booking') ?  __('shop::app.products.book-now') :  __('shop::app.products.add-to-cart') }}</button>
        </form>
