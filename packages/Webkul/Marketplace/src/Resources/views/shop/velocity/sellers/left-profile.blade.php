        ...
        <div class="profile-information-block">
            ...
            <div class="row">
                ...
                @php
                    ...
                    $sellerFlags = app('Webkul\Marketplace\Repositories\SellerFlagReasonRepository')->findWhere(['status'=>1]);
                @endphp
                ...
                <div class="review-info">
                    ...
                    <div class="star-review">
                        <star-ratings ratings="{{ ceil($reviewRepository->getAverageRating($seller)) }}" push-class="mr5"></star-ratings>
                    </div>
            ...
            <div class="row seller-flag">
                <a href="#" @click="showModal('contactForm'); hideModel('reportForm')">{{ __('marketplace::app.shop.sellers.profile.contact-seller') }}</a>
            ...
            <div class="row seller-flag">
                @if (
                    core()->getConfigData('marketplace.settings.seller_flag.enable')
                    && (
                        auth()->guard('customer')->user()
                        || core()->getConfigData('marketplace.settings.seller_flag.guest_can')
                    )
                )
                    <a href="#" @click="showModal('reportForm'); hideModel('contactForm')">{{ core()->getConfigData('marketplace.settings.seller_flag.text') ?: 'Report Seller' }}</a>
                @endif
    ...
    <script type="text/x-template" id="seller-flag-form-template">
        ...
        <form method="POST" action="{{route('marketplace.flag.seller.store')}}" @submit.prevent="onSubmit" style="text-align:left">
            ...
            <div class="control-group mb-2" :class="[errors.has('name') ? 'has-error' : '']">
                ...
                <input v-validate="'required'" v-model="report.name" type="text" class="form-style" id="name" name="name" data-vv-as="&quot;{{ __('marketplace::app.shop.flag.name') }}&quot;" value="{{ old('name') }}"/>
            ...
            <div class="control-group mb-2" :class="[errors.has('email') ? 'has-error' : '']">
                ...
                <input type="email" v-model="report.email" class="form-style" v-validate="'required'" id="email" name="email" data-vv-as="&quot;{{ __('marketplace::app.shop.flag.email') }}&quot;" value="{{ old('email') }}" />
                <span class="control-error" v-if="errors.has('email')">@{{ errors.first('email') }}</span>
            </div>

            <div class="control-group" :class="[errors.has('reason') ? 'has-error' : '']">
                @if (core()->getConfigData('marketplace.settings.seller_flag.reason'))
                    <label for="reason" class="label-style required">{{ __('marketplace::app.shop.flag.reason') }}</label>
                    <select name="reason" id="reason" v-validate="'required'" class="form-style" v-model="reason">
                    ...
                    <textarea class="form-style mt-3" v-validate="'required'" id="other-reason"  v-if="reason == 'other'" name="reason" data-vv-as="&quot;{{ __('marketplace::app.shop.flag.reason') }}&quot;" value="{{ old('reason') }}"></textarea>
                @else
                    <label for="reason" class="label-style required">{{ __('marketplace::app.shop.flag.reason') }}</label>
                    <textarea class="form-style" v-validate="'required'" id="other-reason" name="reason" data-vv-as="&quot;{{ __('marketplace::app.shop.flag.reason') }}&quot;" value="{{ old('reason') }}"></textarea>
                @endif
    ...
    <script>
        Vue.component('seller-flag-form', {
            ...
            created () {
                ...
                @auth('customer')
                    @if(auth('customer')->user())
                        this.report.email = "{{ auth('customer')->user()->email }}";
                        this.report.name = "{{ auth('customer')->user()->first_name }} {{ auth('customer')->user()->last_name }}";
                    @endif
                @endauth
            },
        });
