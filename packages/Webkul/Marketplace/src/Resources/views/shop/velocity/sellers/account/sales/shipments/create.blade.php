...
@push('scripts')
    ...
    <script type="text/x-template" id="order-item-list-template">
        ...
        <div>
            <div class="form-group" :class="[errors.has('shipment[source]') ? 'has-error' : '']">
                <label for="shipment[source]" class="required mandatory">{{ __('marketplace::app.shop.sellers.account.sales.shipments.source') }}</label>

                <select v-validate="'required'" class="form-style" name="shipment[source]" id="shipment[source]" data-vv-as="&quot;{{ __('marketplace::app.shop.sellers.account.sales.shipments.source') }}&quot;" v-model="source">
                                            ...
                                            <tbody>
                                                @foreach (core()->getCurrentChannel()->inventory_sources as $inventorySource)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?php
                                                                $sourceQty = $sellerOrderItem->item->type == 'bundle' ? $sellerOrderItem->item->qty_ordered: 0;
                                                        ...
                                                        <td>
                                                            ...
                                                            <div class="control-group" :class="[errors.has('{{ $inputName }}') ? 'has-error' : '']">
                                                                ...
                                                                <input type="text" v-validate="'required|numeric|min_value:0|max_value:{{$sourceQty}}'" class="control" id="{{ $inputName }}" name="{{ $inputName }}" value="0" data-vv-as="&quot;{{ __('marketplace::app.shop.sellers.account.sales.shipments.qty-to-ship') }}&quot;" :disabled="source != '{{ $inventorySource->id }}' && '{{ $sellerOrderItem->item->type }}' != 'bundle'"/>
