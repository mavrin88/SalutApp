...
@section('page-detail-wrapper')
    ...
    <div class="account-layout">
        ...
        <form method="POST" action="" @submit.prevent="onSubmit" class="account-table-content">
            ...
            <div class="account-table-content">
                ...
                <accordian :title="'{{ __('marketplace::app.admin.products.general') }}'" :active="true">
                    ...
                    <div slot="body">
                        ...
                        <div class="control-group" :class="[errors.has('type') ? 'has-error' : '']">
                            <label for="type" class="required mandatory">{{ __('admin::app.catalog.products.product-type') }}</label>
                            <select class="control" v-validate="'required'" id="type" name="type" {{ $familyId ? 'disabled' : '' }} data-vv-as="&quot;{{ __('admin::app.catalog.products.product-type') }}&quot;">
                                @foreach($sellerAllowedProductTypes as $sellerAllowedProductType)
                                    <option value="{{ $sellerAllowedProductType['key'] }}" {{ $type == $sellerAllowedProductType['key'] ? 'selected' : '' }} >{{ $sellerAllowedProductType['name'] }}</option>
                                @endforeach
                        ...
                        <div class="control-group" :class="[errors.has('attribute_family_id') ? 'has-error' : '']">
                            <label for="attribute_family_id" class="required mandatory">{{ __('admin::app.catalog.products.family') }}</label>
                        ...
                        <div class="control-group" :class="[errors.has('sku') ? 'has-error' : '']">
                            <label for="sku" class="required mandatory">{{ __('admin::app.catalog.products.sku') }}</label>
                ...
                @if ($familyId)
                    ...
                    <accordian :title="'{{ __('marketplace::app.admin.products.configurable-attributes') }}'" :active="true">
                        <div slot="header">
                            {{ __('marketplace::app.admin.products.configurable-attributes') }}
                            <i class="icon expand-icon right"></i>
                        </div> 
                        <div slot="body">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @foreach ($attribute->options as $option)
                                                        <span class="label">
                                                            <input class="form-style" type="hidden" name="super_attributes[{{$attribute->code}}][]" value="{{ $option->id }}"/>
                                                            {{ $option->admin_name }}
