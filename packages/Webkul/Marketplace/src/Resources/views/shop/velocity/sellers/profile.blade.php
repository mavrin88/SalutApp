...
@section('seo')
    <meta name="description"
        content="{{ trim($seller->meta_description) != '' ? $seller->meta_description : Illuminate\Support\Str::limit(strip_tags($seller->description), 120, '') }}" />
...
@section('content-wrapper')
    <div class="main">
        ...
        <div class="profile-details padding-15">
            ...
            @php $reviews = app('Webkul\Marketplace\Repositories\ReviewRepository')->getRecentReviews($seller->id); @endphp

            <div class="profile-details-left-block section">
                @if ($reviews->count())

                    <div class="slider-container">
                        <h5>{{ __('marketplace::app.shop.layouts.reviews') }}</h5>
                        <hr>
                        <div class="slider-content mt-2 mb-5">

                            <carousel :per-page="1" pagination-active-color="#979797" pagination-color="#E8E8E8">
                                @foreach ($reviews as $review)
                                    <slide>
                                        <span class="stars mb-3">
                ...
                <accordian :title="'{{ __('marketplace::app.shop.sellers.profile.return-policy') }}'"
                    :active="false">
                    <div slot="header">
                        {{ __('marketplace::app.shop.sellers.profile.return-policy') }}
                        <i class="icon accordian-icon"></i>
                    </div>
                ...
                <accordian :title="'{{ __('marketplace::app.shop.sellers.profile.shipping-policy') }}'"
                    :active="false">
                    <div slot="header">
                ...
                <accordian :title="'{{ __('marketplace::app.shop.sellers.account.profile.privacy_policy') }}'"
                    :active="false">
                    <div slot="header">
