@if($product->product->type == "booking")
    ...
    <accordian :title="'{{ __('bookingproduct::app.admin.catalog.products.booking') }}'" :active="{{ ($product->product->type == "booking" ? 'true' : 'false') }}">
        <div slot="header">
            {{ __('bookingproduct::app.admin.catalog.products.booking') }}
    ...
    @push('scripts')
        ...
        <script type="text/x-template" id="booking-information-template">
            <div>
                <div class="form-group" :class="[errors.has('booking[type]') ? 'has-error' : '']">
                    ...
                    <select v-validate="'required'" name="booking[type]" v-model="booking.type" class="form-style" data-vv-as="&quot;{{ __('bookingproduct::app.admin.catalog.products.booking-type') }}&quot;" :disabled="! is_new">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="booking[location]" v-model="booking.location" class="form-style"/>
                </div>
            
                <div class="form-group" :class="[errors.has('booking[qty]') ? 'has-error' : '']" v-if="booking.type == 'default' || booking.type == 'appointment' || booking.type == 'rental'">
                    <label class="required">{{ __('bookingproduct::app.admin.catalog.products.qty') }}</label>

                    <input type="text" v-validate="'required|numeric|min:0'" name="booking[qty]" v-model="booking.qty" class="form-style" data-vv-as="&quot;{{ __('bookingproduct::app.admin.catalog.products.qty') }}&quot;"/>
                        
                    <span class="control-error" v-if="errors.has('booking[qty]')">@{{ errors.first('booking[qty]') }}</span>
                </div>

                <div class="form-group" v-if="booking.type != 'event' && booking.type != 'default'" :class="[errors.has('booking[available_every_week]') ? 'has-error' : '']">
                    <label class="required">{{ __('bookingproduct::app.admin.catalog.products.available-every-week') }}</label>

                    <select v-validate="'required'" name="booking[available_every_week]" v-model="booking.available_every_week" class="form-style" data-vv-as="&quot;{{ __('bookingproduct::app.admin.catalog.products.available-every-week') }}&quot;">
                ...
                <div v-if="! parseInt(booking.available_every_week)">
                    <div class="form-group date" :class="[errors.has('booking[available_from]') ? 'has-error' : '']">
                    ...
                    <div class="form-group date" :class="[errors.has('booking[available_to]') ? 'has-error' : '']">
                ...
                <div class="default-booking-section" v-if="booking.type == 'default'">
                    @include ('marketplace::shop.sellers.account.catalog.products.accordians.booking.default', ['bookingProduct' => $bookingProduct])
                </div>

                <div class="appointment-booking-section" v-if="booking.type == 'appointment'">
                    @include ('marketplace::shop.sellers.account.catalog.products.accordians.booking.appointment', ['bookingProduct' => $bookingProduct])
                </div>

                <div class="event-booking-section" v-if="booking.type == 'event'">
                    @include ('marketplace::shop.sellers.account.catalog.products.accordians.booking.event', ['bookingProduct' => $bookingProduct])
                </div>

                <div class="rental-booking-section" v-if="booking.type == 'rental'">
                    @include ('marketplace::shop.sellers.account.catalog.products.accordians.booking.rental', ['bookingProduct' => $bookingProduct])
                </div>

                <div class="table-booking-section" v-if="booking.type == 'table'">
                    @include ('marketplace::shop.sellers.account.catalog.products.accordians.booking.table', ['bookingProduct' => $bookingProduct])
                </div>
        ...
        @include ('marketplace::shop.sellers.account.catalog.products.accordians.booking.slots')
    @endpush
@endif