...
namespace Webkul\Marketplace\Http\Controllers\Shop\Velocity;
        ...
        if ($cart) {
            ...
            foreach ($items as $index => $item) {
                ...
                if (isset($item['additional']['seller_info'])) {
                    $cartItems[$index]['seller_product'] = true;
                    $seller = app('Webkul\Marketplace\Repositories\SellerRepository')->find($item['additional']['seller_info']['seller_id']);

                    $sellerProduct = app('Webkul\Marketplace\Repositories\ProductRepository')->getMarketplaceProductByProduct($item->product->id, $seller->id);
                    $baseSellerProduct = isset($sellerProduct->parent_id) ? $sellerProduct->parent : $sellerProduct;

                    if (
                        isset($sellerProduct->is_owner) &&
                        ! $sellerProduct->is_owner
                    ) {
                        $images = product_image()->getGalleryImages($baseSellerProduct)[0];
                    }
                    $cartItems[$index]['seller_info'] = ['seller' => $seller, 'rating' => app('Webkul\Marketplace\Repositories\ReviewRepository')->getAverageRating($seller)];
                }
