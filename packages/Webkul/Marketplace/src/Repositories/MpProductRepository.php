...
use Webkul\Marketplace\Repositories\ProductVideoRepository;
...
class MpProductRepository extends Repository
{
    public function __construct(
        ...
        protected AttributeOptionRepository $attributeOption,
        protected ProductAttributeValueRepository $attributeValue,
        protected ProductInventoryRepository $productInventory,
    ...
    public function update(array $data, $id, $attribute = "id")
    {
        ...
        if (
            ...
            if (
                $product->parent_id &&
                $this->checkVariantOptionAvailabiliy($data, $product)
            ) {
                $data['parent_id'] = NULL;
            }
            if (empty($data['new'])) {
            ...
            // Get inventory data from inventory table relation
            foreach ($product->inventories as $product_inventory) {
                if ($product_inventory && ($product_inventory->vendor_id == 0)) {
                    $product->inventories = $product_inventory->qty;
                }
            }

            if ($product->type == 'configurable') {
                $this->updateDefaultVariantId($product);
            }
            ...
            foreach ($attributes as $attribute) {
                if (
                    ...
                    (
                        in_array($attribute->type, ['date', 'datetime']) &&
                        ! $data[$attribute->code]
                    )
                ) {
                    continue;
     ...
     * @return void
     */
    public function updateDefaultVariantId($product)
    {
        $defaultVariantId = request()->get('default_variant_id');
        if ($defaultVariantId) {
            $this->setDefaultVariantId($defaultVariantId,$product);
            $product->save();
        }
    }
     ...
     * @return void
     */
    public function setDefaultVariantId($defaultVariantId,$product)
    {
        $product->additional = array_merge($product->additional ?? [], [
    ...
    public function getAll($categoryId = null)
    {
        ...
        $results = app('Webkul\Product\Repositories\ProductFlatRepository')->scopeQuery(function ($query) use ($params, $categoryId) {
            ...
            $qb = $query->distinct()
                        ...
                        ->addSelect(DB::raw('IF( '. $table_prefix .'product_flat.special_price_from IS NOT NULL
