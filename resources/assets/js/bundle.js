...
(function ($) {
	$(function() {
		...
		customSelect('.profile-addresses__list select, .form-item-field-user-bdate-und-0-value select, .cart-block select');
		//endregion

		// $('.form-item-field-user-bdate-und-0-value .date-padding').append('<div class="form-item">\n' +
		// 	'                      <button class="m-calendar-button" type="button">Открыть календарь</button>\n' +
		// 	'                      <div class="m-calendar-wrapper">\n' +
		// 	'                        <div class="m-calendar m-calendar_birthday"></div>\n' +
		// 	'                      </div>\n' +
		// 	'                    </div>');
...
window.addEventListener('load', function () {
	...
	//region Popup календарь
	var btn = document.querySelector('.m-calendar-button');

	if (btn) {
		var calendarEl = btn.nextElementSibling.querySelector('.m-calendar');

		var currentDate = new Date();
		var currentDateFormat = currentDate.toISOString().slice(0, 10);
		var currentDateYear = currentDate.getFullYear();

		// Для дня рождения
		if (calendarEl.classList.contains('m-calendar_birthday')) {
			var birthdayYear = document.querySelector('.form-item-field-user-bdate-und-0-value-year select').value;
			var birthdayMonth = document.querySelector('.form-item-field-user-bdate-und-0-value-month select').value;
			var birthdayDay = document.querySelector('.form-item-field-user-bdate-und-0-value-day select').value;

			// Редкий случай, когда заполнили только год рождения
			if (birthdayYear) {
				currentDateFormat = birthdayYear + '-01-01';
				...
				// текущий год минус 18, формат yyyy-mm-dd
				currentDateFormat = currentDateYear - 18 + '-01-01';
			}
		}

		// Инициализация календарей
		var calendar = new TavoCalendar(calendarEl, {
			locale: 'ru',
			past_select: true,
			date: currentDateFormat
		});

		// Если меняем год в селекте
		const yearField = document.querySelector('.form-item-field-user-bdate-und-0-value-year select');
		if (yearField) {
			yearField.addEventListener('change', (event) => {
				var newCurrentDateFormat = currentDateFormat.substring(4);
				currentDateFormat = event.target.value + newCurrentDateFormat;

				// Очищаем вручную div с календарём
				document.querySelector('.m-calendar_birthday.tavo-calendar').innerHTML = "";

				// Инициализируем новый календарь, т.к. нет методов set
				calendar = new TavoCalendar(calendarEl, {
					locale: 'ru',
					past_select: true,
					date: currentDateFormat
				});
		...
		var formSelects = btn.parentElement.parentElement.querySelectorAll('.form-item select');

		// Для дня рождения
		if (calendarEl.classList.contains('m-calendar_birthday')) {
			formSelects = btn.parentElement.parentElement.parentElement.querySelectorAll('.form-item select');
			var setDate = function setDate(str) {
				var arr = str.split('-').reverse();
				formSelects[0].customSelect.value = parseInt(arr[0]);
				formSelects[1].customSelect.value = parseInt(arr[1]);
				formSelects[2].customSelect.value = parseInt(arr[2]);
			};
		}

		calendarEl.addEventListener('calendar-select', function () {
...
window.addEventListener('load', () => {
	const maskedPhonesInput = document.querySelectorAll('.phone-mask');
...
(function ($) {
	$(function () {
		...
		//region Счетчик добавления в сравнение
		var compare_count = $('.favorite-count').html();
		$(document).on('click', 'a.flag.flag-action', function () {
			console.log('flag');
			$('.favorite-count').html(++compare_count);
		});
		...
		$('.sidebar-block__top').on('click', function () {
			$(this).toggleClass('js-active').parent().find('.sidebar-block__body').toggle();
		});
		...
		//region Добавляем +1 в корзину
		$('.product-block .cart_plus').on('click', function(e){
			e.preventDefault();
		...
		//region Товар, показываем эффекты
		$('.product-full-media__bottom li[class^="n"]').on('hover', function(){
			var index = $(this).attr('class').slice(1);
			$('.product-full-media__effects-big').addClass('js-active').find('li').eq(index-1).addClass('js-active');
			// $('.product__main__media__tabs_image').toggleClass('disabled');
		});
		$('.product-full-media__bottom li[class^="n"]').on('mouseleave', function() {
			var index = $(this).attr('class').slice(1);
			$('.product-full-media__effects-big').removeClass('js-active').find('li').eq(index-1).removeClass('js-active');
		});
		//endregion

		//region Colorbox для эффектов (используется сейчас в node--firework.tpl)
		...
		//region Запуск/пауза видео по клику в любом месте на нём
		$('.product-full-media__video video').on('click', function () {
		...
		$(document).on('click', '[data-popup-video]', function () {
			...
			// Видео
			if (!$videoUrl) {
				$videoUrl = "https://data.salutarium.ru/video_hd/" + $videoId + ".mp4";
			}
			$modal.find('[data-modal-video-player]').html('<video src="' + $videoUrl + '" autoplay="autoplay" controls="controls" disablepictureinpicture="disablePictureInPicture" controlslist="nodownload"></video>');
			// Изображения
			$modal.find('[data-modal-video-image]').attr('src', $productImage);
			// Заголовок
			$modal.find('[data-modal-video-title]').html($productTitle.text());
			// Рейтинг
			if ($productRate.length) {
		...
		$(document).on('click', '.open-preview', function () {
			...
			console.log('$productPrices', $productPrices.html());

			//region Заполнение попапа
			// Изображение
			// $modal.find('img').attr('src', $productImage);
			$modal.find('.image-popup__media').html('<img src="'+$productImage+'" alt="#">');
			// В избранное
			if ($productActions.length) {
				$modal.find('[data-modal-video-actions]').html($productActions.get(0).outerHTML);
			}
		...
		//region Видео в попапе (плагин)
		$.fn.popupVideo = function (options) {

			$(this).click(function (e) {

				var settings = $.extend({
					'path': $(this).attr('data-video-path') ? $(this).attr('data-video-path') : '//media.salutarium.ru/video_other/',
				...
				var description_exist = '';

				if (!$(this).attr('data-video-description')) {
					description_exist = 'video-popup-simple__block_full';
				}

				e.preventDefault();
				$('.video-popup-simple').remove();

				$('body').prepend('<div class="video-popup-simple">\n' +
					'    <div class="video-popup-simple__overlay"></div>\n' +
					'    <div class="video-popup-simple__block ' + description_exist + '">\n' +
					'      <div class="video-popup-simple__heading">\n' +
					'        <div class="video-popup-simple__title"></div>\n' +
					'        <div class="video-popup-simple__close">Закрыть</div>\n' +
					'      </div>\n' +
					'      <div class="video-popup-simple__player">\n' +
					'      </div>\n' +
					'      <div class="video-popup-simple__bottom"></div>\n' +
					'    </div>\n' +
					'  </div>');

				var player = $('.video-popup-simple__player'),
					video_name = $('.video-popup-simple__title'),
					video_description = $('.video-popup-simple__bottom');
				...
				$('.video-popup-simple__close, .video-popup-simple__overlay').on('click', function () {
					$('.video-popup-simple').fadeOut(function () {
						video.trigger('stop');
						$(this).remove();
					})
				});
		...
		// Попап видео
		$('.popup-video').popupVideo({path: 'https://data.salutarium.ru/video_hd/', is_product: true});
		//endregion
...
(function ($) {
	$(function () {
		$('.carousel-banners__list').slick({
			slidesToShow: 1,
			dots: true,
			fade: true,
			prevArrow: '<div class="carousel-banners__arrow carousel-banners__arrow--prev"></div>',
			nextArrow: '<div class="carousel-banners__arrow carousel-banners__arrow--next"></div>',
			// responsive: [
			// 	{
			// 		breakpoint: 1024,
			// 		settings: {
		...
		$('.carousel-product-cards__list').slick({
			slidesToShow: 6,
			arrows: false,
			responsive: [
				{
...
//region Форма для ввода кода
const formCode = document.querySelector('.code-fields');
if (formCode) {
	const inputs = formCode.querySelectorAll('input');
	if (inputs) {
		const KEYBOARDS = {
			backspace: 8,
			arrowLeft: 37,
		...
		formCode.addEventListener('input', handleInput)
...
$(function() {
    if (typeof Swiper !== 'undefined') {
        ...
        const swiperMoreProducts = new Swiper('#more-products .swiper', {
            speed: 400,
            spaceBetween: 21,
            slidesPerView: 1,
            centeredSlides: false,
    ...
    const filtersCatalog = () => {
        const closeBtn = document.querySelector('.category-filters__close');
        const filtersBlock = document.querySelector('.category-filters');
        const filterBtn = document.querySelector('.category-actions__filter');
        if (filtersBlock && closeBtn && filterBtn) {
            const close = () => {
                filtersBlock.classList.remove('js-active');
            }
            const open = () => {
                filtersBlock.classList.add('js-active');
            }
